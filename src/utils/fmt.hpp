...
template <typename T>
struct range_format_kind_ : std::integral_constant<
                                range_format,
...
using range_formatter_type = formatter<remove_cvref_t<decltype(range_mapper<buffered_context<Char>>{}.map(std::declval<Element>()))>, Char>;
...
struct range_format_kind
    : conditional_t<is_range<T, Char>::value, detail::range_format_kind_<T>, std::integral_constant<range_format, range_format::disabled>> {};
...
//     R,
//     Char,
//     enable_if_t<range_format_kind<R, Char>::value == range_format::string || range_format_kind<R, Char>::value == range_format::debug_string>> {
//    private:
//     using range_type = detail::maybe_const_range<R>;
...
//         auto out = ctx.out();
//         if (detail::const_check(range_format_kind<R, Char>::value == range_format::debug_string)) *out++ = '"';
...
struct formatter<
    T,
    Char,
    enable_if_t<conjunction<detail::is_container_adaptor_like<T>, bool_constant<range_format_kind<T, Char>::value == range_format::disabled>>::value>>
